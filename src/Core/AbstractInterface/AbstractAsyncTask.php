...
abstract class AbstractAsyncTask
{
     ...
     */
    abstract function handler(\swooleServer $server, $taskId, $fromId);

    /*
     * 注意   server为主worker进程的server   但taskId为分配该任务的主worker分配的taskId 为每个主worker进程内独立自增
     */
    abstract function finishCallBack(\swooleServer $server, $task_id, $resultData);
    ...
    protected function finish($dataForFinishCallBack = null)
    {
        ...
        Server::getInstance()->getSwooleServer()->finish($this);
    }
}