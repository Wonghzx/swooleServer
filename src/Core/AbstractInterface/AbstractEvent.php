...
abstract class AbstractEvent
{
     ...
     */
    abstract function beforeWorkerStart(\swooleServer $server);
     ...
     * @return    [type]        [description]
     */
    abstract function onStart(\swooleServer $server);
     ...
     */
    abstract function onWorkerStart(\swooleServer $server, $workerId);
     ...
     */
    abstract function onShutdown(\swooleServer $server);


    abstract function onWorkerError(\swooleServer $server, $worker_id, $worker_pid, $exit_code);

    /*
        * 每个worker进程启动均会执行该函数，6个worker就执行6次
        */
    abstract function onWorkerStop(\swooleServer $server, $workerId);

    abstract function onTask(\swooleServer $server, $taskId, $workerId, $callBackObj);

    abstract function onDispatcher(Request $request, Response $response, $targetControllerClass, $targetAction);

    abstract function onFinish(\swooleServer $server, $taskId, $callBackObj);
